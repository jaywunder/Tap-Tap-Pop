...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function () {
	  function Game() {
	    ...
	    this.running = true;
	    this.highScore = parseFloat((0, _util.getCookie)('highScore2')) || 0;
	    this.tickAngle = 0;
	    ...
	    this.container = d3.select('body').append('svg').attr('id', 'draw-shapes').attr('width', window.innerWidth).attr('height', window.innerHeight).style('background-color', GREY);

	    this.shapeContainer = this.container.append('g').attr('transform', 'translate(' + this.centerX + ', ' + this.centerY + ')');

	    this.centerShape = this.shapeContainer.append('circle').attr('r', this.centerRadius).attr('fill', NOCOLOR).style('stroke', GREEN).style('stroke-width', this.thickness);

	    this.targetCircle = this.shapeContainer.append('circle').attr('cy', this.centerRadius).attr('r', this.thickness / 2.5).attr('fill', GREY).attr('transition-timing-function', 'ease-in-out');

	    this.tick = this.shapeContainer.append('rect').attr('x', -this.centerRadius / 20).attr('y', this.centerRadius - this.thickness * 1.2).attr('width', this.centerRadius / 10).attr('height', this.thickness * 2.4).attr('fill', DARKBLUE).attr('rx', 5).attr('ry', 5).attr('transition-timing-function', 'linear');
	    ...
	    window.addEventListener('keydown', function (event) {
	      switch (event.which) {
	        case 32:
	          // space key
	          _this.onSpaceBar();break;
	        case 27:
	          // esc
	          _this.running = !_this.running;break;
	      }
	    });
	  ...
	  _createClass(Game, [{
	    ...
	    value: function onResize(event) {
	      this.container.attr('width', window.innerWidth).attr('height', window.innerHeight);

	      this.shapeContainer.attr('transform', 'translate(' + this.centerX + ', ' + this.centerY + ')');

	      this.centerShape.attr('r', this.centerRadius).style('stroke-width', this.thickness);

	      this.targetCircle.attr('cy', this.centerRadius).attr('r', this.thickness / 2.5);

	      this.tick.attr('x', -this.centerRadius / 20).attr('y', this.centerRadius - this.thickness * 1.2).attr('width', this.centerRadius / 10).attr('height', this.thickness * 2.4);
	    }
	  }, {
	    key: 'onSpaceBar',
	    value: function onSpaceBar() {
	      if (this.tickOverLapsCircle()) {
	        ...
	        if (this.targetAngle < 0) this.targetAngle = 360 + this.targetAngle;
	    ...
	    key: 'moveTargetCircle',
	    value: function moveTargetCircle() {
	      this.targetCircle.transition().attr('transform', 'rotate(' + this.targetAngle + ')').duration(500);
	    }
	  }, {
	    key: 'tickOverLapsCircle',
	    value: function tickOverLapsCircle() {
	      return Math.abs(this.tickAngle) > this.targetAngle - 10 && Math.abs(this.tickAngle) < this.targetAngle + 10;
	    }
	  }, {
	    key: 'updateGame',
	    value: function updateGame() {
	      ...
	      if (this.running) {
	        var tickSpeed = (3 + this.points * 0.1) * this.direction;
	        ...
	        if (this.colorQueue.length > 0 && !this.isTransitioning) {
	          ...
	          this.centerShape.transition().style('stroke', this.colorQueue[0]).duration(300).on('end', function () {
	            _this2.isTransitioning = false;
	            _this2.colorQueue.splice(0, 1);
	          });
	        }

	        this.tick.transition().attr('transform', 'rotate(' + this.tickAngle + ')').duration(15);
	      }
	    }
	  }, {
	    key: 'updateScore',
	    value: function updateScore() {
	      ...
	      (0, _util.setCookie)('highScore2', this.highScore, 9999999999999);
	    ...
	    key: 'centerRadius',
	    get: function get() {
	      return window.innerHeight < window.innerWidth ? window.innerHeight / 6 : window.innerWidth / 6;
	    }
	  }, {
	    key: 'centerX',
	    get: function get() {
	    ...
	    key: 'thickness',
	    get: function get() {
	      return this.centerRadius / 2.5;
	    }
	  }, {
	    key: 'points',
	    set: function set(val) {
	      this._points = val;
	      if (val === 0) this.centerShapeColor = GREEN;
	    },
	    get: function get() {
	      return this._points;
	    }
	  }, {
	    key: 'centerShapeColor',
	    set: function set(val) {
	    ...
	    set: function set(val) {
	      ...
	      if (this._misses === 0) this.centerShapeColor = GREEN;else if (this._misses === 1) this.centerShapeColor = YELLOW;else if (this._misses === 2) this.centerShapeColor = RED;else if (this._misses >= 3) {
	        if (this.points - 1 < 0) this.centerShapeColor = GREEN;else this.points--;
